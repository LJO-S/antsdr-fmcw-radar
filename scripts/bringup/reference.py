...
# Setup contexts
try:
    ctx = iio.Context('ip:192.168.5.10')
except:
...
ctrl.find_channel('RX_LO', True).attrs["frequency"].value = str(int(RXLO))
ctrl.find_channel('TX_LO', True).attrs["frequency"].value = str(int(TXLO))
...
# Plot
if do_plots:
    plt.plot(reals, label="Measured I")
    plt.plot(a >> 4, label="Expected I")
    plt.xlabel("Samples")
