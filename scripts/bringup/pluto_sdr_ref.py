...
class PlutoSdr(object):
    ...
    def rxSynth(self):
        # read only always
        values = [
            x.split(":") for x in self.phy.attrs["rx_path_rates"].value.split(" ")
        ]
    ...
    def txSynth(self):
        # read only always
        values = [
            x.split(":") for x in self.phy.attrs["tx_path_rates"].value.split(" ")
        ]
        in_mhz = [(x[0], int(x[1]) / 1e6) for x in values]
        return dict(in_mhz)
    ...
    def rxStatus(self):
        ...
        print(
            "Rx Fs:{:2.1f}MHz\tBB: {:7.2f}MHz".format(
                self.sampling_frequency, self.rxBBSampling()
            )
        )
        print(
            "   BW:{:2.1f}MHz\tLO: {:7.2f}MHz".format(
                self.rx_bandwidth, self.rx_lo_freq
            )
        )
        print(
            " Gain:{:2.1f}dB\tMode: {:s}".format(
                self.rx_gain, self.phy_rx.attrs["gain_control_mode"].value
            )
        )
    ...
    def _set_rx_gain(self, value=None):
        ...
        if value is None:
            ...
            self.phy_rx.attrs["gain_control_mode"].value = "slow_attack"
        else:
            self.phy_rx.attrs["gain_control_mode"].value = "manual"
            self.phy_rx.attrs["hardwaregain"].value = "{:2.3f} dB".format(value)
    ...
    def txStatus(self, showDds=False):
        """print the key parameters for the receive signal chain"""
        print(
            "Tx Fs:{:5.1f}MHz\tBB: {:7.2f}MHz".format(
                self.sampling_frequency, self.txBBSampling()
            )
        )
        print(
            "   BW:{:5.1f}MHz\tLO: {:7.2f}MHz".format(
                self.tx_bandwidth, self.tx_lo_freq
            )
        )
        print(" Gain:{:4.1f}dB\tState: {:s}".format(self.tx_gain, self._get_tx_state()))
    ...
    def _set_tx_gain(self, value):
        ...
        self.phy_tx.attrs["hardwaregain"].value = "{:2.3f} dB".format(value)
...
if __name__ == "__main__":
    ...
    def _available(ch):
        ans = [
            float(v) / 1e6
            for v in ch.attrs["sampling_frequency_available"].value.split()
        ]
        return ans
    ...
    def querySampling(dev):
        ...
        print(
            " adc1: {:5.2f}MHz available:[{:5.2f}, {:4.2f}]".format(
                _sampling(adc1), *_available(adc1)
            )
        )
        print(
            " adc2: {:5.2f}MHz available:[{:5.2f}, {:4.2f}]".format(
                _sampling(adc2), *_available(adc1)
            )
        )
        print(
            " dac1: {:5.2f}MHz available:[{:5.2f}, {:4.2f}]".format(
                _sampling(dac1), *_available(adc1)
            )
        )
        print(
            " dac2: {:5.2f}MHz available:[{:5.2f}, {:4.2f}]".format(
                _sampling(dac2), *_available(adc1)
            )
        )
